...
namespace BankingSystem
{
    public class CreateBranchService
    {
        public CreateBranchResponse CreateBranch(CreateBranchRequest createBranchRequest)
        {
            ...
            createBranchRequest.PostalCode = createBranchRequest.PostalCode.Trim();
            ...
            while (Inserted == null && CheckBranchInserted < 3)//   The Branch is created or not It will try 3 times
            {            
                Inserted = crudBranch.Create(branchDetails);   // Check if the branch have been created or not
                CheckBranchInserted++;                         // It is for checking three times that the branch will be inserted in the databse r not
