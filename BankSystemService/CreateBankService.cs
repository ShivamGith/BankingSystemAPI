...
namespace BankingSystem
{
    public class CreateBankService
    {
        public CreateBankResponse CreateBank(CreateBankRequest createBankRequest)
        {
            ...
            while (Inserted == null && CheckInserted < 3) // The Bank is created or not It will try 3 times
            {
                Inserted = crudBank.Create(bankDetails);   // Insertion
                CheckInserted++;                           // Checking If The Bank Have been created in the database or not
